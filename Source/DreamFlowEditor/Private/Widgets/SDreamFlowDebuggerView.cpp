...
namespace DreamFlowDebuggerView
{
	...
	static TSharedRef<SWidget> BuildChip(const FText& Text, const FSlateColor& ForegroundColor,
		...
		if (const UDreamFlowAsset* Asset = Executor->GetFlowAsset())
		{
			...
			else
			{
				for (const FDreamFlowVariableDefinition& VariableDefinition : Asset->Variables)
				{
					...
					const bool bIsModified = bHasRuntimeValue && !AreValuesEqual(
						CurrentValue, VariableDefinition.DefaultValue);
					...
					{
						Snapshot += FString::Printf(
							TEXT(" (default: %s)"), *VariableDefinition.DefaultValue.Describe());
					}
...
void SDreamFlowDebuggerView::Tick(const FGeometry& AllottedGeometry, const double InCurrentTime,
...
void SDreamFlowDebuggerView::RefreshExecutors()
{
	...
	const bool bLocalCleanupChanged = PreviousCachedCount != CachedExecutors.Num() || bHadSelectedExecutor !=
		SelectedExecutor.IsValid();
...
void SDreamFlowDebuggerView::RebuildSelectedExecutorDetails()
{
	...
	if (SelectedInfoListView.IsValid())
	{
		SelectedInfoListView->RequestListRefresh();
	}

	if (SelectedVariableListView.IsValid())
	{
		SelectedVariableListView->RequestListRefresh();
	}

	SelectedInspectorContainer->Invalidate(EInvalidateWidgetReason::Layout);
}
...
void SDreamFlowDebuggerView::RefreshSelectedExecutorItems()
{
	...
	{
		TSharedPtr<DreamFlowDebuggerView::FDebuggerInfoItem> Item = MakeShared<
			DreamFlowDebuggerView::FDebuggerInfoItem>();
		Item->Label = Label;
		Item->Value = Value;
	...
	for (const FDreamFlowVariableDefinition& VariableDefinition : Asset->Variables)
	{
		TSharedPtr<DreamFlowDebuggerView::FDebuggerVariableItem> Item = MakeShared<
			DreamFlowDebuggerView::FDebuggerVariableItem>();
		...
		Item->bIsModified = Item->bHasRuntimeValue && !DreamFlowDebuggerView::AreValuesEqual(
			Item->CurrentValue, VariableDefinition.DefaultValue);
...
TSharedRef<SWidget> SDreamFlowDebuggerView::BuildSelectedExecutorInspector()
{
	return SNew(SBorder)
		.BorderImage(FAppStyle::GetBrush("ToolPanel.GroupBorder"))
		.Padding(10.0f)
		[
			...
			.WidgetIndex(this, &SDreamFlowDebuggerView::GetSelectedInspectorWidgetIndex)

			+ SWidgetSwitcher::Slot()
			[
				SNew(STextBlock)
				.Text(FText::FromString(
					TEXT("Select an active executor to inspect runtime variables, context, and execution state.")))
				.TextStyle(FAppStyle::Get(), "SmallText")
				.ColorAndOpacity(FSlateColor(EStyleColor::ForegroundHeader))
				.WrapTextAt(320.0f)
			]
			...
			[
				...
				[
					...
					[
						SNew(SBorder)
						.BorderImage(FAppStyle::GetBrush("WhiteBrush"))
						.BorderBackgroundColor(this, &SDreamFlowDebuggerView::GetSelectedExecutorStateBackgroundColor)
						.Padding(FMargin(6.0f, 2.0f))
						[
							SNew(STextBlock)
							.Text(this, &SDreamFlowDebuggerView::GetSelectedExecutorStateText)
							.TextStyle(FAppStyle::Get(), "SmallText")
							.ColorAndOpacity(this, &SDreamFlowDebuggerView::GetSelectedExecutorStateColor)
						]
					]
				]
				...
				[
					...
					.Padding(1.0f)
					[
						SAssignNew(SelectedInfoListView, SListView<TSharedPtr<DreamFlowDebuggerView::FDebuggerInfoItem>>)
				...
				[
					...
					[
						SNew(STextBlock)
						.Text(this, &SDreamFlowDebuggerView::GetRuntimeVariablesTitleText)
						.TextStyle(FAppStyle::Get(), "SmallText")
						.ColorAndOpacity(FSlateColor(EStyleColor::ForegroundHeader))
					]
					+ SHorizontalBox::Slot()
					...
					[
						SNew(STextBlock)
						.Text(this, &SDreamFlowDebuggerView::GetModifiedVariableSummaryText)
						.TextStyle(FAppStyle::Get(), "SmallText")
						.ColorAndOpacity_Lambda([this]()
						{
							return Algo::CountIf(SelectedVariableItems,
								[](const TSharedPtr<DreamFlowDebuggerView::FDebuggerVariableItem>& Item)
								{
				...
				[
					SNew(SWidgetSwitcher)
					.WidgetIndex(this, &SDreamFlowDebuggerView::GetVariableContentWidgetIndex)
					...
					[
						...
						[
							SNew(STextBlock)
							.Text(FText::FromString(
								TEXT("This flow asset does not define any environment variables.")))
							.TextStyle(FAppStyle::Get(), "SmallText")
							.ColorAndOpacity(FSlateColor(EStyleColor::ForegroundHeader))
							.WrapTextAt(420.0f)
						]
					...
					+ SWidgetSwitcher::Slot()
					[
						SAssignNew(SelectedVariableListView,
						           SListView<TSharedPtr<DreamFlowDebuggerView::FDebuggerVariableItem>>)
...
FText SDreamFlowDebuggerView::GetSelectedExecutorStateText() const
{
	const UDreamFlowExecutor* Executor = GetSelectedExecutor();
...
FSlateColor SDreamFlowDebuggerView::GetSelectedExecutorStateColor() const
{
	const UDreamFlowExecutor* Executor = GetSelectedExecutor();
	return Executor != nullptr
		       ? DreamFlowDebuggerView::GetStateAccentColor(Executor->GetDebugState())
		       : FSlateColor(EStyleColor::ForegroundHeader);
}

FSlateColor SDreamFlowDebuggerView::GetSelectedExecutorStateBackgroundColor() const
{
	return GetSelectedExecutorStateColor().GetSpecifiedColor().CopyWithNewOpacity(0.16f);
}

FText SDreamFlowDebuggerView::GetRuntimeVariablesTitleText() const
{
	const UDreamFlowExecutor* Executor = GetSelectedExecutor();
	const UDreamFlowAsset* Asset = Executor != nullptr ? Executor->GetFlowAsset() : nullptr;
...
FText SDreamFlowDebuggerView::GetModifiedVariableSummaryText() const
{
	const int32 ModifiedVariableCount = Algo::CountIf(SelectedVariableItems,
		[](const TSharedPtr<DreamFlowDebuggerView::FDebuggerVariableItem>& Item)
		{
			return Item.IsValid() && Item->bIsModified;
		});
...
int32 SDreamFlowDebuggerView::GetSelectedInspectorWidgetIndex() const
{
	return GetSelectedExecutor() != nullptr ? 1 : 0;
}

int32 SDreamFlowDebuggerView::GetVariableContentWidgetIndex() const
{
	return SelectedVariableItems.IsEmpty() ? 0 : 1;
}
...
TSharedRef<SWidget> SDreamFlowDebuggerView::BuildVariableRow(const FDreamFlowVariableDefinition& VariableDefinition,
                                                             const FDreamFlowValue& CurrentValue,
                                                             bool bHasRuntimeValue) const
{
	const bool bIsModified = bHasRuntimeValue && !DreamFlowDebuggerView::AreValuesEqual(
		CurrentValue, VariableDefinition.DefaultValue);
	const FText StatusText = !bHasRuntimeValue
		                         ? FText::FromString(TEXT("Default"))
		                         : (bIsModified
			                            ? FText::FromString(TEXT("Modified"))
			                            : FText::FromString(TEXT("Runtime")));

	const FSlateColor StatusColor = !bHasRuntimeValue
		                                ? FSlateColor(EStyleColor::ForegroundHeader)
		                                : (bIsModified
			                                   ? FSlateColor(EStyleColor::Primary)
			                                   : FSlateColor(EStyleColor::AccentGreen));
			...
			[
				...
				[
					...
					[
						...
						.TextStyle(FAppStyle::Get(), "SmallText")
						.ColorAndOpacity(bShowDefaultLine
							                 ? FSlateColor(EStyleColor::ForegroundHeader)
...
TSharedRef<ITableRow> SDreamFlowDebuggerView::GenerateSelectedInfoRow(
	TSharedPtr<DreamFlowDebuggerView::FDebuggerInfoItem> Item, const TSharedRef<STableViewBase>& OwnerTable) const
{
...
TSharedRef<ITableRow> SDreamFlowDebuggerView::GenerateSelectedVariableRow(
	TSharedPtr<DreamFlowDebuggerView::FDebuggerVariableItem> Item, const TSharedRef<STableViewBase>& OwnerTable) const
{
	...
	if (Executor == nullptr)
	{
		return FText::Format(
			FText::FromString(TEXT(
				"Run the flow in PIE or game to inspect active executors, breakpoints, and runtime variables. Active executors: {0}")),
			FText::AsNumber(CachedExecutors.Num()));
	}

	return FText::Format(
		FText::FromString(TEXT("Inspect runtime state for {0}. Active executors: {1}  Selected state: {2}")),
		Executor->GetFlowAsset() != nullptr
			? FText::FromString(Executor->GetFlowAsset()->GetName())
